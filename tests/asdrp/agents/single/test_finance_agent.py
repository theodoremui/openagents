...
#
# Comprehensive tests for FinanceAgent implementation.
#
# Test Coverage:
...
class TestFinanceAgentCreation:
    """
    Test FinanceAgent creation functionality.
    ...
    def test_create_with_default_instructions(self):
        """
        Test that create_finance_agent() uses DEFAULT_INSTRUCTIONS when no instructions provided.
        
        Verifies:
        - Agent is created successfully
        - Agent name is "FinanceAgent"
    ...
    def test_create_with_explicit_none_instructions(self):
        """
        Test that create_finance_agent(None) uses DEFAULT_INSTRUCTIONS.
    ...
    def test_create_with_custom_instructions(self):
        """
        Test that create_finance_agent() accepts and uses custom instructions.
        
        Verifies:
        - Custom instructions are applied correctly
        - Agent name remains "FinanceAgent"
        - Protocol compliance is maintained
        """
        custom_instructions = "You are an expert financial data analyst specializing in stock market analysis."
    ...
    def test_create_with_empty_string_instructions(self):
        """
        Test that create_finance_agent() accepts empty string instructions.
...
class TestFinanceAgentModelConfiguration:
    """
    Test FinanceAgent creation with model configuration.
    ...
    def test_create_with_model_config(self):
        """
        Test that create_finance_agent() applies model configuration correctly.
        ...
        agent = create_finance_agent("Test instructions", model_config)
    ...
    def test_create_with_model_config_and_default_instructions(self):
        ...
        agent = create_finance_agent(None, model_config)
        
        assert agent.instructions == DEFAULT_INSTRUCTIONS
        assert hasattr(agent, 'model')


class TestFinanceAgentProtocolCompliance:
    """
    Test that FinanceAgent properly implements AgentProtocol interface.
...
class TestFinanceAgentTools:
    ...
    def test_tools_are_finance_tools(self):
        """
        Test that agent tools come from FinanceTools.tool_list.
        ...
        # Tools should match FinanceTools.tool_list
        assert agent.tools == FinanceTools.tool_list


class TestFinanceAgentErrorHandling:
    """
    Test FinanceAgent error handling and exception scenarios.
    ...
    def test_import_error_raises_agent_exception(self):
        ...
        - agent_name is set to "finance"
        """
        # Mock FinanceTools import inside the function to raise ImportError
        with patch('asdrp.actions.finance.finance_tools.FinanceTools', side_effect=ImportError("No module 'yfinance'")):
            with pytest.raises(AgentException) as exc_info:
                create_finance_agent("Test")
            
            assert exc_info.value.agent_name == "finance"
    ...
    def test_invalid_model_config_raises_agent_exception(self):
        ...
        with pytest.raises(AgentException) as exc_info:
            create_finance_agent("Test", mock_config)
        
        assert exc_info.value.agent_name == "finance"
        assert "Failed to create" in str(exc_info.value)


class TestFinanceAgentDefaultInstructions:
    """
    Test DEFAULT_INSTRUCTIONS constant.
    
    This class validates the default instructions constant used by FinanceAgent.
    """
    ...
    def test_default_instructions_contains_keywords(self):
        ...
        Verifies the instructions are appropriate for a financial data agent.
        """
        instructions_lower = DEFAULT_INSTRUCTIONS.lower()
        # Should mention financial data, stock, or related concepts
        assert any(keyword in instructions_lower for keyword in [
            "financial", "stock", "market", "ticker", "data"
        ])


class TestFinanceAgentIntegration:
    """
    Integration tests for FinanceAgent.
    ...
    def test_agent_creation_with_all_parameters(self):
        ...
        custom_instructions = "Custom financial data instructions for testing."
        
        agent = create_finance_agent(custom_instructions, model_config)
    ...
    def test_multiple_agent_instances_independent(self):
        ...
        """
        agent1 = create_finance_agent("Instructions 1")
        agent2 = create_finance_agent("Instructions 2")
        
        assert agent1.name == agent2.name == "FinanceAgent"
...
class TestFinanceAgentEdgeCases:
    """
    Test edge cases and boundary conditions for FinanceAgent.
    ...
    def test_very_long_instructions(self):
        ...
        agent = create_finance_agent(long_instructions)
    ...
    def test_instructions_with_special_characters(self):
        ...
        agent = create_finance_agent(special_instructions)
    ...
    def test_unicode_instructions(self):
        ...
        """
        unicode_instructions = "あなたは金融データアシスタントです"  # Japanese: "You are a financial data assistant"
        agent = create_finance_agent(unicode_instructions)
