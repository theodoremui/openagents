...
# test_wiki_agent.py
#
# Comprehensive tests for WikiAgent implementation.
#
# Test Coverage:
...
# - Tool configuration and availability (WikiTools)
...
from asdrp.agents.single.wiki_agent import (
    create_wiki_agent,
    DEFAULT_INSTRUCTIONS,
)


class TestWikiAgentCreation:
    """
    Test WikiAgent creation functionality.
    ...
    def test_create_with_default_instructions(self):
        """
        Test that create_wiki_agent() uses DEFAULT_INSTRUCTIONS when no instructions provided.

        Verifies:
        - Agent is created successfully
        - Agent name is "WikiAgent"
        ...
        assert agent.name == "WikiAgent"
        assert agent.instructions == DEFAULT_INSTRUCTIONS
        assert isinstance(agent, AgentProtocol)

    def test_create_with_explicit_none_instructions(self):
        """
        Test that create_wiki_agent(None) uses DEFAULT_INSTRUCTIONS.

        Verifies that None is treated as "use default" rather than causing an error.
        """
        agent = create_wiki_agent(None)

        assert agent.instructions == DEFAULT_INSTRUCTIONS

    def test_create_with_custom_instructions(self):
        """
        Test that create_wiki_agent() accepts and uses custom instructions.

        Verifies:
        - Custom instructions are applied correctly
        - Agent name remains "WikiAgent"
        - Protocol compliance is maintained
        """
        custom_instructions = "You are an expert Wikipedia research assistant specializing in scientific topics."
        agent = create_wiki_agent(custom_instructions)
    ...
    def test_create_with_empty_string_instructions(self):
        """
        Test that create_wiki_agent() accepts empty string instructions.
...
class TestWikiAgentModelConfiguration:
    """
    Test WikiAgent creation with model configuration.
    ...
    def test_create_with_model_config(self):
        """
        Test that create_wiki_agent() applies model configuration correctly.
        ...
        agent = create_wiki_agent("Test instructions", model_config)
    ...
    def test_create_with_model_config_and_default_instructions(self):
        ...
        agent = create_wiki_agent(None, model_config)
    ...
    def test_create_with_model_config_high_temperature(self):
        ...
        Wikipedia queries typically use moderate temperature for balance.
        """
        model_config = ModelConfig(name="gpt-4", temperature=0.9, max_tokens=4000)
        agent = create_wiki_agent("High creativity instructions", model_config)

        assert agent.name == "WikiAgent"
        assert hasattr(agent, 'model')


class TestWikiAgentProtocolCompliance:
    """
    Test that WikiAgent properly implements AgentProtocol interface.
...
class TestWikiAgentTools:
    """
    Test WikiAgent tool configuration.

    This class validates that the agent has the correct tools configured
    from WikiTools.tool_list.
    """

    def test_agent_has_tools(self):
        """
        Test that WikiAgent has tools configured.
    ...
    def test_tools_are_wikitools(self):
        """
        Test that agent tools come from WikiTools.tool_list.
        ...
        wikitools_tool_names = [tool.name for tool in WikiTools.tool_list]

        assert len(agent.tools) == len(WikiTools.tool_list)
        assert set(agent_tool_names) == set(wikitools_tool_names)

        # Also verify they're functionally equivalent (same tool names in same order)
        assert agent_tool_names == wikitools_tool_names

    def test_tools_include_search(self):
        """
        Test that WikiAgent includes search tool.

        Verifies essential Wikipedia search functionality is available.
        """
        agent = create_wiki_agent()
    ...
    def test_tools_count(self):
        """
        Test that WikiAgent has expected number of tools.

        WikiTools should provide multiple tools (search, summary, content, etc.)
        """
        agent = create_wiki_agent()

        # WikiTools provides 8 tools: search, get_page_summary, get_page_content,
        # get_page_section, get_page_images, get_page_links, set_language, get_random_page
        assert len(agent.tools) >= 8


class TestWikiAgentErrorHandling:
    """
    Test WikiAgent error handling and exception scenarios.
    ...
    def test_import_error_raises_agent_exception(self):
        ...
        - agent_name is set to "wiki"
        """
        # Mock WikiTools import to raise ImportError
        with patch('asdrp.agents.single.wiki_agent.WikiTools', side_effect=ImportError("No module 'wikipedia'")):
            with pytest.raises(AgentException) as exc_info:
                create_wiki_agent("Test")

            assert exc_info.value.agent_name == "wiki"
    ...
    def test_agent_import_error_raises_agent_exception(self):
        ...
        with patch('asdrp.agents.single.wiki_agent.Agent') as mock_agent_class:
    ...
    def test_invalid_model_config_raises_agent_exception(self):
        ...
        with pytest.raises(AgentException) as exc_info:
            create_wiki_agent("Test", mock_config)
...
class TestWikiAgentDefaultInstructions:
    """
    Test DEFAULT_INSTRUCTIONS constant.

    This class validates the default instructions constant used by WikiAgent.
    """
    ...
    def test_default_instructions_contains_keywords(self):
        ...
        Verifies the instructions are appropriate for a Wikipedia agent.
        """
        instructions_lower = DEFAULT_INSTRUCTIONS.lower()
        # Should mention Wikipedia or related concepts
        assert "wikipedia" in instructions_lower or "wiki" in instructions_lower

    def test_default_instructions_contains_tool_guidance(self):
        """
        Test that DEFAULT_INSTRUCTIONS includes guidance on tool usage.

        Verifies instructions help the agent use Wikipedia tools effectively.
        """
        instructions_lower = DEFAULT_INSTRUCTIONS.lower()
        # Should mention search, summary, or content retrieval
        assert "search" in instructions_lower or "summary" in instructions_lower or "article" in instructions_lower

    def test_default_instructions_contains_formatting_guidance(self):
        """
        Test that DEFAULT_INSTRUCTIONS includes formatting guidance.

        Verifies instructions include markdown formatting guidelines.
        """
        instructions_lower = DEFAULT_INSTRUCTIONS.lower()
        assert "markdown" in instructions_lower or "format" in instructions_lower


class TestWikiAgentIntegration:
    """
    Integration tests for WikiAgent.
    ...
    def test_agent_creation_with_all_parameters(self):
        ...
        custom_instructions = "Custom Wikipedia research instructions for testing."

        agent = create_wiki_agent(custom_instructions, model_config)
    ...
    def test_multiple_agent_instances_independent(self):
        ...
        """
        agent1 = create_wiki_agent("Instructions 1")
        agent2 = create_wiki_agent("Instructions 2")

        assert agent1.name == agent2.name == "WikiAgent"
    ...
    def test_agent_with_different_model_configs(self):
        ...
        agent1 = create_wiki_agent("Agent 1", config1)
        agent2 = create_wiki_agent("Agent 2", config2)

        assert agent1.name == agent2.name == "WikiAgent"
        assert agent1 is not agent2


class TestWikiAgentEdgeCases:
    """
    Test edge cases and boundary conditions for WikiAgent.
    ...
    def test_very_long_instructions(self):
        ...
        agent = create_wiki_agent(long_instructions)
    ...
    def test_instructions_with_special_characters(self):
        ...
        agent = create_wiki_agent(special_instructions)
    ...
    def test_unicode_instructions(self):
        ...
        """
        unicode_instructions = "你是维基百科助手"  # Chinese: "You are a Wikipedia assistant"
        agent = create_wiki_agent(unicode_instructions)
    ...
    def test_instructions_with_markdown(self):
        ...
        - **Extract**: Get specific content

        > Important: Always cite sources
        """
        agent = create_wiki_agent(markdown_instructions)
    ...
    def test_model_config_edge_values(self):
        ...
        agent_min = create_wiki_agent("Test", config_min_temp)
        assert agent_min.name == "WikiAgent"

        config_max_temp = ModelConfig(name="gpt-4", temperature=2.0, max_tokens=10000)
        agent_max = create_wiki_agent("Test", config_max_temp)
        assert agent_max.name == "WikiAgent"


class TestWikiAgentDocumentation:
    """
    Test that WikiAgent has proper documentation.
    ...
    def test_create_function_has_docstring(self):
        """
        Test that create_wiki_agent() has a docstring.

        Verifies documentation is available for users.
        """
        assert create_wiki_agent.__doc__ is not None
        assert len(create_wiki_agent.__doc__) > 0
    ...
    def test_docstring_contains_parameters(self):
        ...
        """
        docstring = create_wiki_agent.__doc__
