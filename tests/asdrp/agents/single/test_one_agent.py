...
# test_one_agent.py
#
# Comprehensive tests for OneAgent implementation.
#
# Test Coverage:
...
# - Tool configuration and availability (WebSearchTool)
...
class TestOneAgentCreation:
    """
    Test OneAgent creation functionality.
    ...
    def test_create_with_default_instructions(self):
        """
        Test that create_one_agent() uses DEFAULT_INSTRUCTIONS when no instructions provided.
        
        Verifies:
        - Agent is created successfully
        - Agent name is "OneAgent"
    ...
    def test_create_with_explicit_none_instructions(self):
        """
        Test that create_one_agent(None) uses DEFAULT_INSTRUCTIONS.
    ...
    def test_create_with_custom_instructions(self):
        """
        Test that create_one_agent() accepts and uses custom instructions.
        
        Verifies:
        - Custom instructions are applied correctly
        - Agent name remains "OneAgent"
        - Protocol compliance is maintained
        """
        custom_instructions = "You are a general-purpose research assistant with web search capabilities."
    ...
    def test_create_with_empty_string_instructions(self):
        """
        Test that create_one_agent() accepts empty string instructions.
...
class TestOneAgentModelConfiguration:
    """
    Test OneAgent creation with model configuration.
    ...
    def test_create_with_model_config(self):
        """
        Test that create_one_agent() applies model configuration correctly.
        ...
        agent = create_one_agent("Test instructions", model_config)
    ...
    def test_create_with_model_config_minimal(self):
        """
        Test that create_one_agent() works with minimal model config.
    ...
    def test_create_with_model_config_and_default_instructions(self):
        ...
        agent = create_one_agent(None, model_config)
        
        assert agent.instructions == DEFAULT_INSTRUCTIONS
        assert hasattr(agent, 'model')


class TestOneAgentProtocolCompliance:
    """
    Test that OneAgent properly implements AgentProtocol interface.
...
class TestOneAgentTools:
    """
    Test OneAgent tool configuration.
    
    This class validates that the agent has the correct tools configured.
    OneAgent uses WebSearchTool from the agents library.
    """
    ...
    def test_tools_contains_web_search_tool(self):
        """
        Test that agent tools include WebSearchTool.
        
        Verifies the tools are correctly configured with web search capability.
        """
        from agents import WebSearchTool
        
        agent = create_one_agent()
        
        # Tools should be a list containing WebSearchTool instance
        assert len(agent.tools) > 0
        # Verify at least one tool is a WebSearchTool
        assert any(isinstance(tool, WebSearchTool) for tool in agent.tools)


class TestOneAgentErrorHandling:
    """
    Test OneAgent error handling and exception scenarios.
    ...
    def test_import_error_raises_agent_exception(self):
        ...
        - agent_name is set to "one"
        """
        # Mock WebSearchTool import inside the function to raise ImportError
        with patch('agents.WebSearchTool', side_effect=ImportError("No module 'agents'")):
    ...
    def test_invalid_model_config_raises_agent_exception(self):
        ...
        with pytest.raises(AgentException) as exc_info:
            create_one_agent("Test", mock_config)
        
        assert exc_info.value.agent_name == "one"
        assert "Failed to create" in str(exc_info.value)


class TestOneAgentDefaultInstructions:
    """
    Test DEFAULT_INSTRUCTIONS constant.
    
    This class validates the default instructions constant used by OneAgent.
    """
    ...
    def test_default_instructions_contains_keywords(self):
        ...
        Verifies the instructions are appropriate for a general-purpose web search agent.
        """
        instructions_lower = DEFAULT_INSTRUCTIONS.lower()
        # Should mention web search, information, or related concepts
        assert any(keyword in instructions_lower for keyword in [
            "web", "search", "information", "question"
        ])


class TestOneAgentIntegration:
    """
    Integration tests for OneAgent.
    ...
    def test_agent_creation_with_all_parameters(self):
        ...
        custom_instructions = "Custom web search instructions for testing."
        
        agent = create_one_agent(custom_instructions, model_config)
    ...
    def test_multiple_agent_instances_independent(self):
        ...
        """
        agent1 = create_one_agent("Instructions 1")
        agent2 = create_one_agent("Instructions 2")
        
        assert agent1.name == agent2.name == "OneAgent"
...
class TestOneAgentEdgeCases:
    """
    Test edge cases and boundary conditions for OneAgent.
    ...
    def test_very_long_instructions(self):
        ...
        agent = create_one_agent(long_instructions)
    ...
    def test_instructions_with_special_characters(self):
        ...
        agent = create_one_agent(special_instructions)
    ...
    def test_unicode_instructions(self):
        ...
        """
        unicode_instructions = "あなたは一般的なアシスタントです"  # Japanese: "You are a general assistant"
        agent = create_one_agent(unicode_instructions)
